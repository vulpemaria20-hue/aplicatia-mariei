...
# 2. SISTEM DE SECURITATE (Conform sursei [2])
...
if not st.session_state.login:
    ...
    if st.button("Autentificare"):
        if parola_introdusa == "nutrifit2026": # Parola stabilită în sursă [2]
...
# 4. BAZA DE DATE REȚETE (Kcal/100g extrase din Pag. 32-107 [4-12])
baza_alimente = {
...
# 5. FUNCȚII DE CALCUL CONFORM METODOLOGIEI VASILE BOGDAN
def calcul_rmb(greutate, sex, varsta):
    if varsta >= 65: # RMB Seniori (Pag. 50 [13])
        factor = 0.9 if sex == "Masculin" else 0.8
    else: # RMB Adulți (Pag. 12 [14])
...
with tab1:
    ...
    with col2:
        if tip == "Copil/Adolescent":
            # Necesar caloric fix conform Pag. 26 [15]
            ...
            # CORECȚIE EROARE INDEX: split corect [3] apoi 
            ic = int(activitate.split("(")[3].split("-")) 
            tnc = greutate * ic # Formula GA x IC (Pag. 8 [16])
...
with tab2:
    ...
    st.info("Pierderea corectă implică scăderea greutății simultan cu a taliei [18].")

# TAB 3: PLAN ALIMENTAR (Logica de calcul automata din sursa [19])
with tab3:
